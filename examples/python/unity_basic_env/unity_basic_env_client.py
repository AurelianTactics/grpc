...
def generate_agent_action(obs):
  ...
  if obs is None or obs < 10:
    action *= action
  return action


def generate_step_message(action):
  messages = [make_unity_basic_env_note("step", action, 0., False)]
  for msg in messages:
    print("Generating step message {} aO/r/d {}/{}/{} ".format(msg.message, msg.actionObs, msg.reward, msg.done))
    yield msg
...
def unity_basic_env_step(stub, agent):
  ...
  for response in responses:
    if response.message == "step" or response.message == "reset":
      ...
      print("different message type received {}".format(response.message))
...
def unity_basic_env_start(stub, agent):
  ...
  for response in responses:
    if response.message == "reset" or response.message == "step":
      print("Receiving message {} o/r/d {}/{}/{} ".format(response.message, response.actionObs, response.reward,
                                                          response.done))
...
def run():
  ...
  with grpc.insecure_channel('localhost:30051') as channel:
    ...
    # time.sleep(3)
    # unity_basic_env_step(stub, agent)
    print("-------------- End UnityBasicEnv Client --------------")
